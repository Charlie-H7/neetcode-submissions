class Solution {
    ...
    // ------------------------------------------------------------------------------------ //
        ...
        return Math.max(free.at(-1), cooldown.at(-1));

        // Then I'm willing to be we loop over them one more time when complete to choose the best path whose valid state transitions lead us to a max
